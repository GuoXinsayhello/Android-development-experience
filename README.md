...
###使用URl访问网络资源
URl可以由协议名、主机、端口和资源组成，URLConnection表示应用程序和URL之间的通信连接，最常用的Http请求无非是get和post，get请求可以获取静态页面，也可以把参数放在URL字串后面，传递给servlet，post与get的不同之处在于post的参数不是放在URL字串里面，而是放在http请求的正文内。
####post方式与get方式的区别
